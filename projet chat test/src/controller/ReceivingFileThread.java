...
public class ReceivingFileThread extends Thread {
    ...
    public void sendToServer(String line) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "Server férmé,message non envoyé!", "Error", JOptionPane.ERROR_MESSAGE);
        } catch (IOException ex) {
    ...
    public void run() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(frameToDisplayDialog, "Fichier télécharger dans\n"+myDownloadFolder + "\\" + fileName, "Info", JOptionPane.INFORMATION_MESSAGE);
        } catch (IOException ex) {
            Logger.getLogger(ReceivingFileThread.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println("Cette socket est férmé !");
        } finally {
            try {
